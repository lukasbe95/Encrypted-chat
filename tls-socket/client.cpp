...
public:
    ...
    uint16_t get_port()
    {
        if (const char* env_p = std::getenv("EC_PORT"))
        {
            if (std::atoi(env_p) != 0)
            {
                return (uint16_t)std::atoi(env_p);
            }
        }
        return 1234;
    }
...
private:
    ...
    {
        addrinfo hints{0};
        hints.ai_family = AF_UNSPEC; // socket for any address family (ipv4 and v6)
